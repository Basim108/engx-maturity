...
const SCHEMA_ARRAY_OF_METRICS_FILE_PATH = path.resolve(__dirname, '../../data/schemas/arrayOfMetrics.schema.json');
const SCHEMA_ARRAY_OF_RULES_FILE_PATH = path.resolve(__dirname, '../../data/schemas/arrayOfRules.schema.json');
const SCHEMA_METRICS_FILE_PATH = path.resolve(__dirname, '../../data/schemas/metric.schema.json');
...
describe('Validate rule files', () => {
    ...
    test('should match the metrics schema', () => {
        const arrayOfMetricsSchema = JSON.parse(fs.readFileSync(SCHEMA_ARRAY_OF_METRICS_FILE_PATH, 'utf8'));
        const metricsSchema = JSON.parse(fs.readFileSync(SCHEMA_METRICS_FILE_PATH, 'utf8'));
        const typesSchema = JSON.parse(fs.readFileSync(SCHEMA_TYPES_FILE_PATH, 'utf8'));

        const ajv = new Ajv({allErrors: true, schemas: [metricsSchema, typesSchema]});
        const validator = ajv.compile(arrayOfMetricsSchema);

        const metricsFiles = glob.sync(METRIC_FILE_PATTERN, {cwd: RULES_FOLDER_PATH});
